const { PRESALE_PERIOD, PRESALE_STATE, PRESALE_MAX_GOAL, PRESALE_MIN_GOAL } = require('@1hive/apps-marketplace-shared-test-helpers/constants')
...
contract('Presale, refund() functionality', ([anyone, appManager, buyer1, buyer2, buyer3, buyer4, buyer5]) => {
  const itAllowsBuyersToGetRefunded = startDate => {
    ...
    describe('When purchases have been made and the sale is ready to be closed', () => {
      before(async () => {
        this.presale.mockSetTimestamp(startDate)
        await this.contributionToken.generateTokens(buyer4, PRESALE_MAX_GOAL)
        await this.contributionToken.approve(this.presale.address, PRESALE_MAX_GOAL, { from: buyer4 })
